...
Entity* mushroom_green_small_new(Vector3D position, Vector3D velocity)
{
    ...
    if (!ent)
    {
        slog("Failed to create mushroom_green_small");
        return NULL;
    }

    ent->model = gf3d_model_load("mushroom_green_small"); //change to mushroom //mushroom_green_small


    ent->height = 9;
    ent->width = 9;
    ent->depth = 9;
    ent->initSize = GetOrigModelSize("models/mushroom_green_small.obj");
    //ent->size.x = 8; //height
    //ent->size.y = 16; //width
    //ent->size.z = 16; //depth
    ...
    ent->think = mushroom_green_small_think;
    ent->type = 1; //monster
