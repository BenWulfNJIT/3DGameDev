...
void mushroom_green_think(Entity* self);

Entity* mushroom_green_new(Vector3D position)
{
    ...
    if (!ent)
    {
        slog("Failed to create mushroom_green");
        return NULL;
    }

    ent->model = gf3d_model_load("mushroom_green"); //change to mushroom //mushroom_green


    ent->height = 18;
    ent->width = 18;
    ent->depth = 18;
    ent->initSize = GetOrigModelSize("models/mushroom_green.obj");
    //ent->size.x = 8; //height
    //ent->size.y = 16; //width
    //ent->size.z = 16; //depth
    ...
    ent->think = mushroom_green_think;
    ent->type = 1; //monster
