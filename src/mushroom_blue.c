...
void mushroom_blue_think(Entity* self);

Entity* mushroom_blue_new(Vector3D position)
{
    ...
    if (!ent)
    {
        slog("Failed to create mushroom_blue");
        return NULL;
    }

    ent->model = gf3d_model_load("mushroom_blue"); //change to mushroom //mushroom_blue


    ent->height = 10;
    ent->width = 16;
    ent->depth = 16;
    ent->initSize = GetOrigModelSize("models/mushroom_blue.obj");
    //ent->size.x = 8; //height
    //ent->size.y = 16; //width
    //ent->size.z = 16; //depth
